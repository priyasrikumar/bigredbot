...
const delayedDelete = async (message, userID, channelID) => {
    ...
    try {
        ...
        console.error('BigRedBot/index.js:44 There was an error deleting messages.' + ('\n' + error).replace(/\n/g, '\n\t'));
    }
}
...
bot.on('messageReactionAdd', async (reaction, user) => {
    if(user != bot.user && reaction.message.id == config.roleMessageID) {
        var guildUser = bot.guilds.cache.get(config.master_guild).members.cache.get(user.id);
        var role = roles[reaction.emoji.name];
        if (guildUser != null && role != null) {
            guildUser.roles.add(roles[reaction.emoji.name].role);
        }
    }
    try {
...
bot.on('messageReactionRemove', async (reaction,user) => {
    if(user != bot.user &&reaction.message.id == config.roleMessageID) {
        ...
        var role = roles[reaction.emoji.name];
        if (guildUser != null && role != null) {
            guildUser.roles.remove(roles[reaction.emoji.name].role);
        }
    }
})
