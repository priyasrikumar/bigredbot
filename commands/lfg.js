...
const postInvite = async (channel, args, invite) => {
	...
	try {
		...
		embed = {
			...
			timestamp: new Date(),
			footer: {
				text: channel.id === config.master_channel ? 'React with ❌ to delete this message' : 'Head over to the CU server to make your own posts!'
			}
		}
	} catch(e) {
		throw 'BigRedBot/commands/lfg.js:10 There was an error creating the embed.' + ('\n' + e).replace(/\n/g, '\n\t');
	}
	...
	try {
		...
		throw `BigRedBot/commands/lfg.js:29 There was an error sending to ${channel.name} (${channel.id} in ${channel.guild.name}) ${('\n' + e).replace(/\n/g, '\n\t')}`;
	}
}
...
module.exports = {
	...
	async execute(msg, args) {
		...
		try {
			...
			throw 'BigRedBot/commands/lfg.js:86 There was an error creating the invite.' + ('\n' + e).replace(/\n/g, '\n\t');
		}
		...
		try {
			...
			throw 'BigRedBot/commands/lfg.js:92 There was an error sending the embed.' + ('\n' + e).replace(/\n/g, '\n\t');
		}
